  ...
  return (
    <div style={{minHeight:'100vh',background:bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',fontFamily:'-apple-system,BlinkMacSystemFont,SF Pro Display,Segoe UI,sans-serif',padding:'0 20px',transition:'background 0.3s'}}>
      ...
      <div style={{width:'100%',maxWidth:400,background:surface,borderRadius:20,border:`1px solid ${border}`,padding:'28px 24px',boxShadow:dark?'0 24px 64px rgba(0,0,0,0.4)':'0 24px 64px rgba(127,119,221,0.08)'}}>
        ...
        <p style={{fontSize:11,color:txtMuted,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:20}}>crea il tuo account</p>

        <form onSubmit={handleRegister}>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:10,color:txtMuted,letterSpacing:'0.08em',textTransform:'uppercase',display:'block',marginBottom:6}}>nome completo</label>
            <input type="text" value={fullName} onChange={e=>setFullName(e.target.value)} required placeholder="Mario Rossi"
              style={{width:'100%',padding:'12px 14px',border:`1px solid ${border}`,borderRadius:10,fontSize:14,fontFamily:'inherit',background:inputBg,color:txt,outline:'none'}}/>
          </div>
          ...
          <div style={{marginBottom:24}}>
            ...
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={6} placeholder="••••••••"
              style={{width:'100%',padding:'12px 14px',border:`1px solid ${border}`,borderRadius:10,fontSize:14,fontFamily:'inherit',background:inputBg,color:txt,outline:'none'}}/>
            <p style={{fontSize:11,color:txtMuted,marginTop:5}}>minimo 6 caratteri</p>
          </div>
        ...
        <div style={{textAlign:'center',marginTop:20,paddingTop:20,borderTop:`1px solid ${border}`}}>
          <p style={{fontSize:13,color:txtMuted}}>
            hai già un account?{' '}
            <Link href="/login" style={{color:'#7F77DD',textDecoration:'none',fontWeight:500}}>accedi</Link>
          </p>
        </div>
      </div>
